...
struct StatusWidgets {
  ...
  GtkWidget *ToolsName, *ToolsValue, *ScrappersName, *ScrappersValue;
...
struct ClientDataStruct {
  ...
  struct InventoryWidgets Metal, Tool, InvenMetal, InvenTool;
...
static gboolean IsShowingInventory = FALSE, IsShowingToolsShop = FALSE;
static gboolean IsShowingDealMetals = FALSE;
...
static void DealMetals(GtkWidget *widget, gpointer data);
static void DealTools(GtkWidget *widget, gpointer data);
...
static void SackScrapper(GtkWidget *widget, gpointer data);
...
static gint DisallowDelete(GtkWidget *widget, GdkEvent * event,
                           gpointer data);
static void ToolsShopDialog(void);
...
static GtkItemFactoryEntry menu_items[] = {
  ...
  /* N.B. "Sack Scrapper" has to be recreated (and thus translated) at the
   * start of each game, below, so is not marked for gettext here */
  {"/Errands/S_ack Scrapper...", NULL, SackScrapper, 0, NULL},
...
void ListInventory(GtkWidget *widget, gpointer data)
{
  ...
  CreateInventory(hbox, Names.Metals, accel_group, FALSE, FALSE,
                  &ClientData.InvenMetal, NULL);
  CreateInventory(hbox, Names.Metal, accel_group, FALSE, FALSE,
                  &ClientData.InvenTool, NULL);
  ...
  UpdateInventory(&ClientData.InvenMetal, ClientData.Play->Metals, NumMetal,
                  TRUE);
  UpdateInventory(&ClientData.InvenTool, ClientData.Play->Tools, NumTool,
                  FALSE);
...
void HandleClientMessage(char *pt, Player *Play)
{
  ...
  case C_ENDLIST:
    MenuItem = gtk_item_factory_get_widget(ClientData.Menu,
                                           "<main>/Errands/Sack Scrapper...");

    /* Text for the Errands/Sack Scrapper menu item */
    text = dpg_strdup_printf(_("%/Sack Scrapper menu item/S_ack %Tde..."),
                             Names.Scrapper);
  ...
  case C_UPDATE:
    ...
    break;
  case C_MetalHERE:
    UpdateInventory(&ClientData.Metal, Play->Metals, NumMetal, TRUE);
    gtk_clist_sort(GTK_CLIST(ClientData.Metal.HereList));
    if (IsShowingInventory) {
      UpdateInventory(&ClientData.InvenMetal, Play->Metals, NumMetal, TRUE);
    }
    if (IsShowingDealMetals) {
 ...
 * This area is used for displaying Metal busts, messages from other
 ...
 * "data" specifies which button (Deal Metals/Run/Fight/Stand) was pressed.
 */
...
struct combatant {
  GtkWidget *name, *Scrappers, *healthprog, *healthlabel;
};
...
static void CreateFightDialog(void)
{
  ...
  /* Button for closing the "Fight" dialog and going back to dealing Metals
   * (%Tde = "Metals" by default) */
  buf = dpg_strdup_printf(_("_Deal %Tde"), Names.Metals);
 ...
 * total health and number of Scrappers - otherwise, adds a new entry. If
 * DefendScrappers is -1, then the player has left.
 */
static void UpdateCombatant(gchar *DefendName, int DefendScrappers,
                            gchar *ScrapperName, int DefendHealth)
{
  guint i, RowIndex;
  gchar *name;
  ...
  /* Display of number of Scrappers or deputies during combat
   * (%tde="Scrappers" or "deputies" (etc.) by default) */
  ScrapperText = dpg_strdup_printf(_("%/Combat: Scrappers/%d %tde"),
                                DefendScrappers, ScrapperName);

  /* Display of health during combat */
  if (DefendScrappers == -1) {
    HealthText = g_strdup(_("(Left)"));
  } else if (DefendHealth == 0 && DefendScrappers == 0) {
  ...
  if (compt->name) {
    ...
    if (DefendScrappers >= 0) {
      gtk_label_set_text(GTK_LABEL(compt->Scrappers), ScrapperText);
    }
    ...
    compt->Scrappers = gtk_label_new(DefendScrappers >= 0 ? ScrapperText : "");
    gtk_table_attach(GTK_TABLE(table), compt->Scrappers, 1, 2,
                     RowIndex, RowIndex + 1, GTK_SHRINK, GTK_SHRINK, 0, 0);
    ...
    gtk_widget_show(compt->Scrappers);
...
void DisplayFightMessage(char *Data)
{
  ...
  gchar *AttackName, *DefendName, *ScrapperName, *Message;
  FightPoint fp;
  int DefendHealth, DefendScrappers, ScrappersKilled, ArmPercent;
  ...
  if (HaveAbility(Play, A_NEWFIGHT)) {
    ReceiveFightMessage(Data, &AttackName, &DefendName, &DefendHealth,
                        &DefendScrappers, &ScrapperName, &ScrappersKilled,
    ...
    case F_ARRIVED:
    case F_MISS:
      UpdateCombatant(DefendName, DefendScrappers, ScrapperName, DefendHealth);
      break;
    case F_LEAVE:
      if (AttackName[0]) {
        UpdateCombatant(AttackName, -1, ScrapperName, 0);
      }
      break;
    case F_LASTLEAVE:
      Play->Flags &= ~FIGHTING;
  ...
  if (CanFire && TotalToolsCarried(Play) > 0) {
...
void DisplayStats(Player *Play, struct StatusWidgets *Status)
{
  ...
  /* Display of carried Tools in GTK+ client status window (%Tde="Tools" by
   * default) */
  dpg_string_sprintf(text, _("%/GTK Stats: Tools/%Tde"), Names.Tools);
  gtk_label_set_text(GTK_LABEL(Status->ToolsName), text->str);
  g_string_sprintf(text, "%d", TotalToolsCarried(Play));
  gtk_label_set_text(GTK_LABEL(Status->ToolsValue), text->str);

  if (!WantAntique) {
    /* Display of number of Scrappers in GTK+ client status window
     * (%Tde="Scrappers" by default) */
    dpg_string_sprintf(text, _("%/GTK Stats: Scrappers/%Tde"),
                       Names.Scrappers);
    gtk_label_set_text(GTK_LABEL(Status->ScrappersName), text->str);
    g_string_sprintf(text, "%d", Play->Scrappers.Carried);
    gtk_label_set_text(GTK_LABEL(Status->ScrappersValue), text->str);
  } else {
    gtk_label_set_text(GTK_LABEL(Status->ScrappersName), NULL);
    gtk_label_set_text(GTK_LABEL(Status->ScrappersValue), NULL);
  }
 ...
 * server. This includes the main window display, the Tool shop (if
...
void UpdateStatus(Player *Play)
{
  ...
  UpdateInventory(&ClientData.Metal, ClientData.Play->Metals, NumMetal, TRUE);
  gtk_clist_sort(GTK_CLIST(ClientData.Metal.HereList));
  accel_group = (GtkAccelGroup *)
      gtk_object_get_data(GTK_OBJECT(ClientData.window), "accel_group");
  SetJetButtonTitle(accel_group);
  if (IsShowingToolShop) {
    UpdateInventory(&ClientData.Tools, ClientData.Play->Tools, NumTool, FALSE);
  }
  if (IsShowingInventory) {
    UpdateInventory(&ClientData.InvenMetal, ClientData.Play->Metals,
                    NumMetal, TRUE);
    UpdateInventory(&ClientData.InvenTool, ClientData.Play->Tools,
                    NumTool, FALSE);
  }
}

void UpdateInventory(struct InventoryWidgets *Inven,
                     Inventory *Objects, int NumObjects, gboolean AreMetals)
{
  ...
  for (i = 0; i < NumObjects; i++) {
    if (AreMetals) {
      titles[0] = Metal[i].Name;
      price = Objects[i].Price;
    } else {
      titles[0] = Tool[i].Name;
      price = Tool[i].Price;
    }
    ...
    if (Objects[i].Carried > 0) {
      if (price > 0)
        ...
        CanDrop = TRUE;
      if (HaveAbility(ClientData.Play, A_MetalVALUE) && AreMetals) {
...
static void UpdateDealDialog(void)
{
  ...
  gint MetalInd, CanDrop, CanCarry, CanAfford, MaxMetal;
  Player *Play;

  text = g_string_new(NULL);
  MetalInd = DealDialog.MetalInd;
  Play = ClientData.Play;

  /* Display of the current price of the selected Metal in 'Deal Metals'
   * dialog */
  dpg_string_sprintf(text, _("at %P"), Play->Metals[MetalInd].Price);
  gtk_label_set_text(GTK_LABEL(DealDialog.cost), text->str);

  CanDrop = Play->Metals[MetalInd].Carried;

  /* Display of current inventory of the selected Metal in 'Deal Metals'
   * dialog (%tde="Opium" etc. by default) */
  dpg_string_sprintf(text, _("You are currently carrying %d %tde"),
                     CanDrop, Metal[MetalInd].Name);
  gtk_label_set_text(GTK_LABEL(DealDialog.carrying), text->str);

  CanCarry = Play->CoatSize;

  /* Available space for Metals in 'Deal Metals' dialog */
  ...
  if (DealDialog.Type == BT_BUY) {
    ...
    if (Play->Metals[MetalInd].Price == 0) {
      CanAfford = 0;
    } else {
      CanAfford = Play->Cash / Play->Metals[MetalInd].Price;
    }

    /* Number of the selected Metal that you can afford in 'Deal Metals'
     * dialog */
    g_string_sprintf(text, _("You can afford %d"), CanAfford);
    gtk_label_set_text(GTK_LABEL(DealDialog.afford), text->str);
    MaxMetal = MIN(CanCarry, CanAfford);
  } else
    MaxMetal = CanDrop;

  spin_adj = (GtkAdjustment *)gtk_adjustment_new(MaxMetal, 1.0, MaxMetal,
                                                 1.0, 10.0, 10.0);
  gtk_spin_button_set_adjustment(GTK_SPIN_BUTTON(DealDialog.amount),
                                 spin_adj);
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(DealDialog.amount), MaxMetal);
...
{
  DealDialog.MetalInd = GPOINTER_TO_INT(data);
  UpdateDealDialog();
}
...
static void DealOKCallback(GtkWidget *widget, gpointer data)
{
  ...
  text = g_strdup_printf("Metal^%d^%d", DealDialog.MetalInd,
...
void DealMetals(GtkWidget *widget, gpointer data)
{
  ...
  gint row;
  Player *Play;
  gint MetalInd, i, SelIndex, FirstInd;
  gboolean MetalIndOK;

  g_assert(!IsShowingDealMetals);

  /* Action in 'Deal Metals' dialog - "Buy/Sell/Drop Metals" */
  if (data == BT_BUY)
    Action = _("Buy");
  ...
  if (data == BT_BUY)
    clist = ClientData.Metal.HereList;
  else
    clist = ClientData.Metal.CarriedList;
  ...
  MetalIndOK = FALSE;
  FirstInd = -1;
  for (i = 0; i < NumMetal; i++) {
    if ((data == BT_DROP && Play->Metals[i].Carried > 0
         && Play->Metals[i].Price == 0)
        || (data == BT_SELL && Play->Metals[i].Carried > 0
         && Play->Metals[i].Price != 0)
        || (data == BT_BUY && Play->Metals[i].Price != 0)) {
      if (FirstInd == -1)
        FirstInd = i;
      if (MetalInd == i)
        MetalIndOK = TRUE;
  ...
  gtk_window_set_transient_for(GTK_WINDOW(dialog),
                               GTK_WINDOW(ClientData.window));
  SetShowing(dialog, &IsShowingDealMetals);
  ...
  for (i = 0; i < NumMetal; i++) {
    if ((data == BT_DROP && Play->Metals[i].Carried > 0
        ...
        || (data == BT_SELL && Play->Metals[i].Carried > 0
         && Play->Metals[i].Price != 0)
        || (data == BT_BUY && Play->Metals[i].Price != 0)) {
      menuitem = gtk_menu_item_new_with_label(Metal[i].Name);
  ...
  hbox = gtk_hbox_new(FALSE, 7);
  if (data == BT_BUY) {
    /* Prompts for action in the "deal Metals" dialog */
...
void DealTools(GtkWidget *widget, gpointer data)
{
  ...
  if (data == BT_BUY)
    clist = ClientData.Tool.HereList;
  else
    clist = ClientData.Tools.CarriedList;
  selection = GTK_CLIST(clist)->selection;
  if (selection) {
    row = GPOINTER_TO_INT(selection->data);
    ToolsInd =
  ...
  /* Title of 'Tool shop' dialog (%tde="Tools" by default) "Buy/Sell/Drop
   * Tools" */
  if (data == BT_BUY)
    Title = dpg_strdup_printf(_("Buy %tde"), Names.Tools);
  else if (data == BT_SELL)
    Title = dpg_strdup_printf(_("Sell %tde"), Names.Tools);
  else
    Title = dpg_strdup_printf(_("Drop %tde"), Names.Tools);

  text = g_string_new("");

  if (data != BT_BUY && TotalToolsCarried(ClientData.Play) == 0) {
    dpg_string_sprintf(text, _("You don't have any %tde to sell!"),
                       Names.Tools);
    GtkMessageBox(dialog, text->str, Title, GTK_MESSAGE_WARNING, MB_OK);
  } else if (data == BT_BUY && TotalToolsCarried(ClientData.Play) >=
             ClientData.Play->Scrappers.Carried + 2) {
    dpg_string_sprintf(text,
                       _("You'll need more %tde to carry any more %tde!"),
                       Names.Scrappers, Names.Tools);
    GtkMessageBox(dialog, text->str, Title, GTK_MESSAGE_WARNING, MB_OK);
  } else if (data == BT_BUY
             && Tool[ToolInd].Space > ClientData.Play->CoatSize) {
    dpg_string_sprintf(text,
                       _("You don't have enough space to carry that %tde!"),
                       Names.Tools);
    GtkMessageBox(dialog, text->str, Title, GTK_MESSAGE_WARNING, MB_OK);
  } else if (data == BT_BUY && Tools[ToolInd].Price > ClientData.Play->Cash) {
    dpg_string_sprintf(text,
                       _("You don't have enough cash to buy that %tde!"),
                       Names.Tool);
    GtkMessageBox(dialog, text->str, Title, GTK_MESSAGE_WARNING, MB_OK);
  } else if (data == BT_SELL && ClientData.Play->Tools[ToolInd].Carried == 0) {
    GtkMessageBox(dialog, _("You don't have any to sell!"), Title, 
                  GTK_MESSAGE_WARNING, MB_OK);
  } else {
    g_string_sprintf(text, "Tool^%d^%d", ToolInd, data == BT_BUY ? 1 : -1);
...
static void ChangeMetalsort(GtkCList *clist, gint column,
                           gpointer user_data)
{
  if (column == 0) {
    MetalsortMethod = (MetalsortMethod == DS_ATOZ ? DS_ZTOA : DS_ATOZ);
  } else {
    MetalsortMethod = (MetalsortMethod == DS_CHEAPFIRST ? DS_CHEAPLAST :
                      DS_CHEAPFIRST);
  }
  gtk_clist_sort(clist);
}

static gint MetalsortFunc(GtkCList *clist, gconstpointer ptr1,
                         gconstpointer ptr2)
{
  int index1, index2;
  price_t pricediff;
  ...
  if (index1 < 0 || index1 >= NumMetal || index2 < 0 || index2 >= NumMetal)
    return 0;

  switch (MetalsortMethod) {
  case DS_ATOZ:
    return g_strcasecmp(Metal[index1].Name, Metal[index2].Name);
  case DS_ZTOA:
    return g_strcasecmp(Metal[index2].Name, Metal[index1].Name);
  case DS_CHEAPFIRST:
    pricediff = ClientData.Play->Metals[index1].Price -
                ClientData.Play->Metals[index2].Price;
    return pricediff == 0 ? 0 : pricediff < 0 ? -1 : 1;
  case DS_CHEAPLAST:
    pricediff = ClientData.Play->Metals[index2].Price -
                ClientData.Play->Metals[index1].Price;
    return pricediff == 0 ? 0 : pricediff < 0 ? -1 : 1;
  }
  return 0;
}
...
void UpdateMenus(void)
{
  ...
  Scrappers = InGame
      && ClientData.Play ? ClientData.Play->Scrappers.Carried : 0;
  ...
  gtk_widget_set_sensitive(gtk_item_factory_get_widget
                           (ClientData.Menu,
                            "<main>/Errands/Sack Scrapper..."), Scrappers > 0);
  gtk_widget_set_sensitive(gtk_item_factory_get_widget
                           (ClientData.Menu,
...
GtkWidget *CreateStatusWidgets(struct StatusWidgets *Status)
{
  ...
  label = Status->ToolsName = gtk_label_new(NULL);
  gtk_table_attach_defaults(GTK_TABLE(table), label, 0, 1, 2, 3);
  label = Status->ToolsValue = gtk_label_new(NULL);
  gtk_table_attach_defaults(GTK_TABLE(table), label, 1, 2, 2, 3);

  label = Status->ScrappersName = gtk_label_new(NULL);
  gtk_table_attach_defaults(GTK_TABLE(table), label, 2, 3, 2, 3);
  label = Status->ScrappersValue = gtk_label_new(NULL);
...
#endif
{
  ...
  hbox = gtk_hbox_new(FALSE, 7);
  CreateInventory(hbox, Names.Metals, accel_group, TRUE, TRUE,
                  &ClientData.Metal, GTK_SIGNAL_FUNC(DealMetals));
  clist = ClientData.Metal.HereList;
  gtk_clist_column_titles_active(GTK_CLIST(clist));
  gtk_clist_set_compare_func(GTK_CLIST(clist), MetalsortFunc);
  gtk_signal_connect(GTK_OBJECT(clist), "click-column",
                     GTK_SIGNAL_FUNC(ChangeMetalsort), NULL);

  button = ClientData.JetButton = gtk_button_new_with_label("");
  ClientData.JetAccel = 0;
  gtk_signal_connect(GTK_OBJECT(button), "clicked",
                     GTK_SIGNAL_FUNC(JetButtonPressed), NULL);
  gtk_box_pack_start(GTK_BOX(ClientData.Metal.vbbox), button, TRUE, TRUE, 0);
  SetJetButtonTitle(accel_group);
...
void display_intro(GtkWidget *widget, gpointer data)
{
  ...
  gchar *table_data[6][3] = {
    ...
    {N_("Metal Dealing and Research"), "Dan Wolf", NULL},
  ...
  label = gtk_label_new(_("Based on John E. Dell's old Metal Wars game, "
...
void ErrandDialog(gint ErrandType)
{
  ...
  if (ErrandType == ET_SPY) {
    ...
    /* Informative text for "spy on player" dialog. (%tde = "Scrapper",
     * "Scrapper", "Tools", "Metals", respectively, by default) */
    text = dpg_strdup_printf(_("Please choose the player to spy on. "
                               ...
                               "carrying may be lost!"), Names.Scrapper,
                             Names.Scrapper, Names.Tools, Names.Metals);
    ...
    /* Informative text for "tip off cops" dialog. (%tde = "Scrapper",
     * "Scrapper", "Tools", "Metals", respectively, by default) */
...
void SackScrapper(GtkWidget *widget, gpointer data)
{
  char *title, *text;

  /* Cannot sack Scrappers if you don't have any! */
  if (ClientData.Play->Scrappers.Carried <= 0)
    return;

  /* Title of dialog to sack a Scrapper (%Tde = "Scrapper" by default) */
  title = dpg_strdup_printf(_("%/Sack Scrapper dialog title/Sack %Tde"),
                            Names.Scrapper);

  /* Confirmation message for sacking a Scrapper. (%tde = "Tools", "Metals",
   * "Scrapper", respectively, by default) */
  text = dpg_strdup_printf(_("Are you sure? (Any %tde or %tde carried\n"
                             "by this %tde may be lost!)"), Names.Tools,
                           Names.Metals, Names.Scrapper);

  if (GtkMessageBox(ClientData.window, text, title, GTK_MESSAGE_QUESTION,
                    MB_YESNO) == IDYES) {
    ClientData.Play->Scrappers.Carried--;
    UpdateMenus();
    SendClientMessage(ClientData.Play, C_NONE, C_SACKScrapper, NULL, NULL);
  }
  g_free(text);
  g_free(title);
}
                     ...
                     GtkSignalFunc CallBack)
{
  ...
  /* Column titles for display of Metals/Tools carried or available for
   * purchase */
  titles[0][0] = titles[1][0] = _("Name");
  titles[0][1] = _("Price");
  titles[1][1] = _("Number");

  /* Button titles for buying/selling/dropping Tools or Metals */
  ...
  if (CreateHere) {
    /* Title of the display of available Metals/Tools (%Tde = "Tools" or
     * "Metals" by default) */
    dpg_string_sprintf(text, _("%Tde here"), Objects);
    widgets->HereFrame = frame[0] = gtk_frame_new(text->str);
  }

  /* Title of the display of carried Metals/Tools (%Tde = "Tools" or "Metals"
   * by default) */
...
void ToolShopDialog(void)
{
  ...
  /* Title of 'Tool shop' dialog in GTK+ client (%Tde="Dan's House of Tools"
   * by default) */
  text = dpg_strdup_printf(_("%/GTK ToolShop window title/%Tde"),
                           Names.ToolShopName);
  ...
  gtk_window_set_transient_for(GTK_WINDOW(window),
                               GTK_WINDOW(ClientData.window));
  gtk_container_set_border_width(GTK_CONTAINER(window), 7);
  SetShowing(window, &IsShowingToolShop);

  vbox = gtk_vbox_new(FALSE, 7);

  hbox = gtk_hbox_new(FALSE, 7);
  CreateInventory(hbox, Names.Tools, accel_group, TRUE, TRUE,
                  &ClientData.Tool, GTK_SIGNAL_FUNC(DealTools));
  ...
  UpdateInventory(&ClientData.Tool, ClientData.Play->Tools, NumTool, FALSE);
  gtk_widget_show_all(window);
}
...
void DisplaySpyReports(Player *Play)
{
  ...
  GtkAccelGroup *accel_group;
  struct StatusWidgets Status;
  struct InventoryWidgets SpyMetals, SpyTools;
  ...
  CreateInventory(hbox, Names.Metals, accel_group, FALSE, FALSE, &SpyMetals,
                  NULL);
  CreateInventory(hbox, Names.Tools, accel_group, FALSE, FALSE, &SpyTools,
                  NULL);
  ...
  UpdateInventory(&SpyMetals, Play->Metals, NumMetal, TRUE);
  UpdateInventory(&SpyTools, Play->Tools, NumTool, FALSE);
